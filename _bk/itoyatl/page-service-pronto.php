<?php get_header(); ?>
<!-- start contents -->
            <main class="contents">
                <div class="contents-inner">
                    <div class="pageHeading">
                        <h1>Service</h1>
                    </div>
                    <div class="breadcrumbs">Home - Service</div>
                    <article class="serviceContents">

                        <section class="wrapper-top">
                            <div class="wrapper-top-left">
                                <h2>PRONTO</h2>
                                <div class="serviceContents-logo pronto00"></div>
                                <p class="serviceContents-text">プロントが目指しているのは、イタリアの「バール」、イギリスの「パブ」に見られるような、お客様との親密で活気あるお付き合いです。昼間はカフェとしての、夜はバーとしての顔を持ち、お客様にとってなじみの深い憩いの場を提供するための努力を重ねています。
                                </p>
                            </div>
                            <div class="wrapper-top-right">
                                <div class="serviceContents-image01 pronto01"></div>
                            </div>
                        </section>
                        <section class="wrapper-bottom">
                            <div class="wrapper-bottom-top">
                                <div class="wrapper-bottom-right">
                                    <p class="serviceContents-text">
                                        伊藤屋のプロントは社員、スタッフが分け隔てなく楽しく働く環境を創造しております。①スタッフ一人ひとりがやりがい、テーマを持ちながら、②お客様とのコミュニケーション会話、③感動を与える商品を提供すること、を通して成長出来る店舗作りがテーマです。
                                    </p>
                                </div>

                            <div class="wrapper-bottom-left">
                                <div class="imageWrap">
                                    <div class="serviceContents-image02 pronto02"></div>
                                    <div class="serviceContents-image03 pronto03"></div>
                                </div>
                            </div>
                            </div>
                            <div class="wrapper-bottom-top">
                                <div class="serviceContents-image04 pronto04"></div>
                            </div>
                        </section>
                    </article>
                    <div class="serviceFooter">
                        <nav class="serviceFooter-nav">
                            <ul>
                                <li>
                                    <a href="https://itoya1889.com/service-doutor/">
                                        ドトールコーヒーショップ<span class="arw"></span>
                                    </a>
                                </li>
                                <li>
                                    <a href="https://itoya1889.com/service-otoya/">
                                        大戸屋ごはん処<span class="arw"></span>
                                    </a>
                                </li>
                                <li>
                                    <a href="https://itoya1889.com/service-pronto/">
                                    PRONTO<span class="arw"></span>
                                    </a>
                                </li>
                                <li>
                                    <a href="https://itoya1889.com/service-jittan/">
                                        焼肉&BAR 十炭<span class="arw"></span>
                                    </a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                </div>
            </main>
            <!-- end contents -->
<?php get_footer(); ?>
